...
public class WatchServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet WatchServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet WatchServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user == null) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Thiếu viewer");
            return;
        }
        ...
        try {
            ...
            // Gửi đối tượng content sang JSP
            request.setAttribute("content", content);
            double progress = progressDAO.getProgress(user.getUser_id(), contentId);
            request.setAttribute("watchedSeconds", progress);
